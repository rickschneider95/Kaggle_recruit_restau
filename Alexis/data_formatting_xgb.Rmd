...
air_data <- left_join(air_data,date_info,by="visit_date") %>%
  ...
  rename(holiday_flg_visit = holiday_flg) # CAREFUL, here it's an inner join, some days there were just no restaurants open
...
#Holiday feels -> It's beginning to look a lot like Christmas
...
#Group restaurants by class
```{r}
typesRest <- unique(air_data$genre_name)
typesRest
# GROUPS
group1 <- c("Italian/Franch","International cuisine","Western food","Other")
group2 <- c("Okonomiyaki/Monja/Teppanyaki","Izakaya","Japanese food","Asian")
...
-> create groups of types of restaurants, to push some types that could receive different ppl
