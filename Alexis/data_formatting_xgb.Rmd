...
lag_transf <- function(data){
  data <- data %>%
    group_by(id) %>% 
    mutate(lag_1 = lag(visitors, 1),  #looks at the visitors 1 row before the visitors we are currently looking at.
...
sample_try <- lag_transf(sample_try)
```
...
-> create mean visitors per month or and year variable and categorize it if two many levels? (round it!) *OVERFITTING/DONT*
