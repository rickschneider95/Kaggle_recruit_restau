...
rmonth_transf <- function(data){
  one_h <- data
  one_h$month_reserve <- as.factor(one_h$month_reserve) #extract the month number in the date (eg December is 12)
  one_h <- one_h %>% select(month_reserve)
  one_h <- one_hot(as.data.table(one_h))
  data <- data %>% cbind(one_h)
}
```
...
vmonth_transf <- function(data){
  one_h <- data
  one_h$month_visit <- as.factor(month_visit) #extract the month number in the date (eg December is 12)
  one_h <- one_h %>% select(month_visit)
  one_h <- one_hot(as.data.table(one_h))
  data <- data %>% cbind(one_h)
}
```
...
ryear_transf <- function(data){
  one_h <- data
  one_h$year_reserve <- as.factor(year_reserve)
...
vyear_transf <- function(data){
  one_h <- data
  one_h$year_visit <- as.factor(year_visit)
...
dn_transf <- function(data){
  one_h <- data_air %>%
    ...
    mutate(visit_time = as.factor(case_when(visit_time >= 4 & visit_time <= 16 ~ "noon",          # Lunch
                                                                          TRUE ~ "night")))       # Dinner
  one_h <- one_hot(as.data.table(one_h))
  ...
  append <- data %>%
    ...
    mutate(var_1h_noon = sum(var_1h_noon),
          var_1h_night = sum(var_1h_night)) %>%
    distinct(id,visit_date,var_1h_noon,var_1h_night)

  data <- data %>% cbind(append)
}
