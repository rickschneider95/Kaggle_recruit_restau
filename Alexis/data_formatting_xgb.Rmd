...
While looking at this I found something wierd, since we decided not to use the reservation info since we can't get them from the sample submission, we only need the air_visit_data and air_store_info and potentially the HPG data if we assume that the reserve visitors actually correspond to the real visitors (everyone showed up !)
...
levels(as.factor(ttry$nb_res)) # different nb of reservations in the data! note: info coming from visit data is already summed so it's normal to get only one reservation that's huge...
```
...
For now I'll go on assuming that the reserve info is useless and only construct on varaibles that we can retrieve from the info in the sample submission. You will still find functions that can work with the reserve data but I won't be using them, at least for now.
...
ryear_transf <- function(data){
  one_h <- data
  one_h$reserve_date <- as.factor(str_sub(as.character(one_h$reserve_date), start = 1, end = 4))
  one_h <- one_h %>% select(reserve_date)
  one_h <- one_hot(as.data.table(one_h))
  data <- data %>% cbind(one_h)
}
...
vyear_transf <- function(data){
  one_h <- data
  one_h$visit_date <- as.factor(str_sub(as.character(one_h$visit_date), start = 1, end = 4))
  one_h <- one_h %>% select(visit_date)
  one_h <- one_hot(as.data.table(one_h))
  data <- data %>% cbind(one_h)
}
...
dn_transf <- function(data){
    n <- nrow(dataset)
    counts <- rep(1,n)
    counts <- counts %>% cbind(dataset)
    counts %>% group_by(visit_date,id) %>% mutate(. = sum(.))
    dataset %>% group_by()
    mutate(visit_hour = as.integer(str_sub(as.character(visit_time),
                                  start = 0,
                                  end = 3))) %>%
    mutate(visit_hour = as.factor(case_when(visit_time >= 4 & visit_time <= 16 ~ "noon",          # Lunch
                                                                          TRUE ~ "night")))       # Dinner
  one_h <- one_hot(as.data.table(one_h))
  data <- data %>% cbind(one_h)
}
...
summary(is.na(tttry))
levels(as.factor(tttry$nb_res))
levels(as.factor(ttttry$nb_res))
levels(as.factor(ttttry$av_per_day))
levels(as.factor(data_air$visitors))

data_air %>% filter(visitors == "877")
air_visit_data %>%  filter(visitors == "877")
```
...
```
 ...
 Here is the code to transform it:
...
write.csv(try, "data/data_1h_xgb_improved_1.csv", row.names = FALSE)

trytry <- read.csv("data/data_1h_xgb_improved_1.csv")
