...
lon_transf <- function(data){
  one_h <- data
  one_h$longitude <- as.factor(round(one_h$longitude,1))
...
lat_transf <- function(data){
  one_h <- data
  one_h$latitude <- as.factor(round(one_h$latitude,1))
...
average_transf <- function(data){
  one_h <- data %>%
    mutate(av_per_day = as.factor(round(visitors/nb_res))) %>%
    select(av_per_day) %>%
  one_h <- one_hot(as.data.table(one_h))
  data <- data %>% cbind(one_h)
}
...
write.csv(try, "data/data_1h_xgb_CORRECT.csv", row.names = FALSE)

trytry <- read.csv("data/data_1h_xgb_CORRECT.csv")
